...
const callServiceIngestNewStoriesFromHNJob = new cron.CronJob(process.env.SOURCE_HN_CRON_TIME || '* * * * *', () => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const requestUrl = `${NODE_ENV === 'development'
            ? 'http://localhost:3000'
        ...
        }), totalStoriesSaved = (yield response.json())
            .data.total, msToComplete = Date.now() - startTime;
        // TODO: save msToComplete to ingest status/performance time series
        console.info(`completed scheduled service call for ingesting new stories from HN in ${msToComplete}ms, saving ${totalStoriesSaved} new stories`);
    }
    catch (error) {
        console.error(error);
    }
